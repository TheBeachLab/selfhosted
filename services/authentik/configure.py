#!/usr/bin/env python3
"""Configure the private seven-site passkey gateway via Authentik's local API.
...
def items(path):
    return api(path + '?page_size=200')['results']

def upsert(path, key, value, data):
    old = next((item for item in items(path) if item.get(key) == value), None)
    if old:
        return api(f'{path}{old.get("pk", old.get("brand_uuid"))}/', data, 'PATCH')
...
for slug, name in [('deutsch', 'Deutsch Sprint'), ('whisper', 'Whisper'), ('tts', 'Qwen3-TTS'), ('drop', 'Drop'), ('browser', 'Navegador remoto')]:
    app = upsert('core/applications/', 'slug', slug, {'name': name, 'meta_launch_url': f'https://beachlab.org/{slug}/', 'group': 'Servidor'})
    if not any(x['target'] == app['pk'] and x.get('user') == user['pk'] for x in items('policies/bindings/')):
        api('policies/bindings/', {'target': app['pk'], 'user': user['pk'], 'order': 0, 'failure_result': False})
...
api(f'outposts/instances/{outpost["pk"]}/', {'providers': providers, 'config': config}, 'PATCH')
brand = items('core/brands/')[0]
api(f'core/brands/{brand["brand_uuid"]}/', {'branding_title': 'Beachlab', 'flow_authentication': flow['pk']}, 'PATCH')
print('Configured: fran, passkey-only flow, seven applications, three restricted providers.')
